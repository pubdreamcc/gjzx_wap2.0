<template>
  <div class="examread clearMargin_top">
    <div class="header"><a href="javascript:;" @click="back"><img src="../assets/imgs/live_direction_left@2x.png"></a><span>{{score}}</span></div>
    ...
    <div class="btn" ref="btn" @click="goReExamination">立即补考</div>
  </div>
</template>
...
export default {
  ...
  mounted () {
    // 发送Ajax请求得到考试结果信息
    axios.get(`http://www.gk0101.com/exam/rest/v1/knowPoint/getChaperName?courseId=${this.$route.query.courseId}&userId=${this.$route.query.userId}&institutionId=10103`).then(res => {
      const ret = res.data.data
      this.courseName = ret.courseName
    })
    axios.get(`http://www.gk0101.com/exam/rest/v1/exam/stuPaper?examType=${this.$route.query.examType}&courseId=${this.$route.query.courseId}&institutionId=10103&userId=${this.$route.query.userId}&chapterId=${this.$route.query.chapterId}&id=${this.$route.query.examSheetId}`).then(res => {
      const ret = res.data.data
      if (ret.examPaperDto.score >= 80) {
        this.score = `${ret.examPaperDto.score}分`
        this.$refs.btn.style.display = 'none'
      } else if (ret.examPaperDto.score > 0) {
        this.score = `${ret.examPaperDto.score}分（不通过）`
        this.$refs.btn.style.display = ''
      } else {
        this.score = `0分（不通过）`
        this.$refs.btn.style.display = ''
      }
      this.knowPointName = ret.examPaperDto.examName
